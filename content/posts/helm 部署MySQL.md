...
前面几天煞费苦心的在Ubuntu上部署了kubernetes, 具体怎么部署的请出门右转到 [Linux常用工具使用]()

Kubernets集群有了，就得在Kubernets上部署点什么东西是吧。MySQL作为一名合格的Backend，应该都知道。所以就尝试部署一下MySQL。

有使用Kubernets经验的人都知道，Kubernets有很多的yaml文件需要编写，deployment，service等yaml文件。还是比较繁琐的。所以出现了Helm这个工具。helm这个其实没什么高级的，就是将deployment，service等yaml文件抽象成模版，在这些模版中使用变量，使用helm可以编辑这些变量这样就能很快的在kubernets中进行部署了。

<!--more-->

在继续之前，最好了解一下Helm，kubernetes存储这些知识点
...
我们在master上使用了existingClaim参数，由于我们使用的local存储，所以需要提前创建pv，pvc

```yaml
# mysql-pv-local-claim.yaml
apiVersion: v1
kind: PersistentVolume
metadata:
  name: mysql-primary-pv-local
spec:
  capacity:
    storage: 10Gi
...
$ kubectl apply -f mysql-pv-local-claim.yaml
$ kubectl apply -f mysql-pv-local-claim.yaml
```

其次我们修改了Probe的时间，我在测试的时候默认时间太短了，需要改得稍微长一点，要不然probe失败会导致一直重启


这里有个疑问为什么primary有existingClaim的配置，而secondary没有existingClaim的配置。

参考[Issue1501](https://github.com/bitnami/charts/pull/4501)

* Remove chart and slave PVC (we only need the master PVC since slave only replicates data):

最后helm进行install

```
helm install -f values.yaml --namespace infra dm-mysql bitnami/mysql
```
